    ...
    for i, cfg in enumerate(CONFIGS, 1):
        ...
        row = {
            ...
            **f0_stats(out_wav),
            ...
            "file": f"outputs/seedvc_sweep/{tag}.wav",
        }
        results.append(row)
...
def build_report(base: dict, rows: list[dict]) -> str:
    ...
    lines = [
        "# Seed-VC 表达力参数扫描报告",
        "",
        f"- 源：`jay_0.wav`（7.2s）  目标参考：`trump_0.wav`  diffusion_steps=10",
    ...
    lines += [
        ...
        "- 主观听感以 `outputs/seedvc_sweep/` 下各 wav 为准，建议按 plain → style_sim07 → style_temp14 顺序盲听。",
    ]
    return "\n".join(lines) + "\n"
