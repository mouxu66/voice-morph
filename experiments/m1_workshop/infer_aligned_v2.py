# 用对齐版 QLoRA adapter 推理，验证是否干净（非电音）
...
ADAPTER = "D:/变声/tts_trial/Qwen3-TTS/finetuning/output_qlora_aligned_v2/adapter-final"
REF     = "D:/变声/media/voicebank/merg_004/reference_24k.wav"
SPK     = "meituan_kangaroo"
OUT     = "D:/变声/outputs/ft_aligned_v2"
os.makedirs(OUT, exist_ok=True)
LOG     = "D:/变声/output_qlora_aligned_v2_infer.log"
...
log("[load] base ...")
qwen3tts=Qwen3TTSModel.from_pretrained(BASE,torch_dtype=torch.bfloat16,attn_implementation="sdpa",quantization_config=bnb)
base=qwen3tts.model
def _gi(self): return self.talker.model.codec_embedding
def _si(self,v): self.talker.model.codec_embedding=v
base.get_input_embeddings=_gi.__get__(base); base.set_input_embeddings=_si.__get__(base)

use_lora = not os.environ.get("NO_LORA")
if use_lora:
    log("[load] 套对齐 adapter ...")
...
log(f"[anchor] codec_embedding[3000] <- 袋鼠声纹")
