...
def test_binding_warning_silent_when_bound_correctly(_wechat_running, monkeypatch):
    monkeypatch.setattr(wp, "input_device_probe",
                        lambda: {"device": "CABLE Output (VB-Audio Virtual Cable)",
                                "file": "f", "hits": 1})
...
def test_prepare_env_mode_zero_surfaces_binding_warning(monkeypatch):
    ...
    monkeypatch.setattr(wv, "_need_wechat_restart", lambda: (False, "VM_WECHAT_RESTART=0 已关闭重启"))
    monkeypatch.setattr(wv, "_binding_warning", lambda: "微信上次录音用的是「麦克风阵列」→ 这条语音可能录成静音")
    info = wv._prepare_recording_env()
    assert info["restart"] is False
