...
def atlas_png(ffmpeg, tmp_path_factory):
    ...
    _run(ffmpeg, ["-f", "lavfi", "-i", "testsrc2=size=512x144:rate=1",
                  "-frames:v", "1", str(p)])
    return p
...
def gif_file(ffmpeg, tmp_path_factory):
    ...
    _run(ffmpeg, ["-f", "lavfi", "-i", "testsrc2=size=64x48:rate=4",
                  "-t", "1", "-loop", "0", str(p)])
    return p


def test_atlas_8x9_slices_rows(atlas_png, tmp_path):
    meta = {"id": "poke", "name": "Poké", "category": "卡通", "license": "MIT",
            "attribution": "测试作者 poke",
            "frameW": 64, "frameH": 48, "cols": 8,
...
def test_atlas_bad_size_rejected(tmp_path):
    meta = {"id": "bad", "name": "Bad", "category": "x", "license": "x",
            "frameW": 200, "frameH": 200, "cols": 8}
    out = tmp_path / "bad"
    out.mkdir()
    # 缺 atlas 文件直接报错
...
def test_gif_multi_splits_frames(gif_file, tmp_path):
    ...
    out = tmp_path / "cat"
    meta = {"id": "cat", "name": "猫", "category": "像素萌宠", "license": "CC0",
            "gif_map": {"idle": "anim.gif", "play": "anim.gif", "think": "anim.gif",
                        "listen": "anim.gif", "build": "anim.gif", "error": "anim.gif"},
            "frameW": 0, "frameH": 0}
...
def test_skin_json_config_handed_offs(tmp_path):
    ...
    meta = {"id": "cfg", "name": "Cfg", "category": "其他", "license": "Apache-2.0",
            "frameW": 150, "frameH": 150}
    rows = {"idle": {"sheet": "idle.webp", "frames": 4, "dur": 2.0},
            "listen": {"sheet": None},
...
def test_validate_skin_files_missing_sheet(tmp_path):
    meta = {"id": "hole", "name": "Hole", "category": "x", "license": "x",
            "frameW": 150, "frameH": 150}
    write_skin_json(tmp_path, meta, {
...
def test_pack_gif_skin_expired_names(gif_file, tmp_path):
    ...
    meta = {"id": "ghost", "name": "Ghost", "category": "x", "license": "x",
