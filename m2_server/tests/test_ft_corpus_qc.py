...
def _write_wav(path: Path, seconds: float = 3.0, amp: float = 0.3, sr: int = 24000, seed: int = 0):
...
def test_report_flags_silent_clips_as_d(ft):
    ...
    assert any(("静音" in r["reason"] or "有效语音" in r["reason"]) for r in rep["top_reasons"])
