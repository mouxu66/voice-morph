...
思路（全程不 Hook、不注入微信，只模拟人手操作）：
    ...
    2. 前台化微信聊天窗口，按住 Alt 触发微信 4.1.9+ 的官方「发送语音消息」开始录音
    ...
    4. 松开 Alt → 微信结束录音并把录到的内容作为语音消息发出（单条最长 60s）
...
重要更正（实测踩坑）：
    ...
    - 微信 4.1.9+ 才上线「发送语音消息」：按住 Alt 说话、松开发送（或点输入框
      右下角话筒）。本模块默认用 Alt；若你改过该快捷键，调 VM_WECHAT_RECORD_KEY。
"""
...
# 发语音方式：微信 4.1.9+ 的「发送语音消息」= 按住 Alt 说话、松开发送。
# 默认 alt；若微信里改过该快捷键，用 VM_WECHAT_RECORD_KEY 指定键名（见 _record_key_code）。
RECORD_KEY = os.environ.get("VM_WECHAT_RECORD_KEY", "alt")
LEAD_S = float(os.environ.get("VM_WECHAT_LEAD_S", "0.35"))   # 按住 Alt 后等待录音开始
TAIL_S = float(os.environ.get("VM_WECHAT_TAIL_S", "0.3"))    # 播完后的尾巴静音（松键前）
# 自动按键流程失败时是否自动降级为「引导式手动发送」（播放到 CABLE + 用户自己按 Alt）
...
_vk_menu = 0x12   # Alt：微信「发送语音消息」的按住说话键
_keyup = 0x0002


def _record_key_code() -> int:
    """把 RECORD_KEY 映射成虚拟键码。默认 alt（微信发语音）；支持常见别名。"""
...
def _trigger_record() -> None:
    """按住 Alt 开始录制语音消息（微信 4.1.9+「发送语音消息」）。"""
    _key(_record_key_code())


def _finish_record() -> None:
    """松开 Alt 结束录制并发送语音消息。"""
    _key(_record_key_code(), up=True)
...
def _do_send(req: SendVoiceReq):
    ...
    try:
        ...
        # 3) 微信前台 → 按住 Alt 开始录制语音消息
        _foreground_wechat()
        _trigger_record()
        steps.append("已按住 Alt 开始录音")
        ...
        time.sleep(TAIL_S)
        _finish_record()
        steps.append("已松开 Alt，语音已发送")
        time.sleep(0.6)
    except Exception as exc:
        # 失败也要：①松开 Alt（防止按住不放卡键）②还原声卡（reset 兜底）
        try:
            _key(_record_key_code(), up=True)
        except Exception:
            pass
    ...
    return {"ok": True, "outcome": "ok", "wav": wav.name, "duration_s": round(duration, 1),
            "steps": steps, "restored": restored, "restore_error": restore_err,
...
def _guided_fallback(wav: Path, duration: float, steps: list[str]) -> dict:
    ...
    try:
        ...
        _play_to_cable(wav, duration)
        _safe_restore()
        return {"steps": ["已降级：音频已播到 CABLE，请到微信按住 Alt 录完松开发送"],
                "hint": "请到微信按住 Alt 说话，录到这段音频后松开 Alt 发送",
                "lead_s": 2.0}
    except Exception as e:
        return {"steps": ["降级播放失败"],
