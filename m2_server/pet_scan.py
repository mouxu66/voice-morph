...
def _gh_get(path: str, params: dict | None = None) -> dict | None:
    ...
    if r.status_code == 403:
        ...
        if rem is not None and rem.isdigit() and int(rem) <= 0:
            raise ScanError(
                "GitHub API 限流已耗尽。可设置环境变量 GITHUB_TOKEN 提升配额，"
                "或稍等一分钟后再试。"
            )
...
def _log_fail(repo_full: str, path: str, exc: Exception) -> None:
    ...
    with _SCAN_LOCK:
        if len(_SCAN["fails"]) < FAIL_LOG_MAX:
            _SCAN["fails"].append({"repo": repo_full, "path": path, "reason": str(exc)[:300]})
        _SCAN["built_fail"] += 1
...
def find_repos() -> list[dict]:
    ...
    for q in _SEED_QUERIES:
        if len(seen) >= MAX_REPOS:
            break
        data = _gh_get(
            "/search/repositories", {"q": q, "sort": "stars", "order": "desc", "per_page": 8}
        )
...
def _repo_tree(repo: dict) -> list[dict] | None:
    ...
    data = _gh_get(
        f"/repos/{repo['full_name']}/git/trees/{repo['default_branch']}", {"recursive": "1"}
    )
...
def _gh_download(repo: dict, path: str, dst: Path, box: dict) -> None:
    ...
    try:
        r = requests.get(url, headers=headers, timeout=(CONNECT_TIMEOUT, READ_TIMEOUT), stream=True)
    except requests.RequestException as exc:
...
def _try_build(kind: str, repo: dict, cand: dict, tmp: Path) -> dict:
    ...
    if kind == "gif":
        paths = cand.get("paths") or [cand.get("path")]
        srcs: list[Path] = []
        fnames: list[str] = []
        for p in paths:
    ...
    return {
        ...
        "bundle": False,
        "source_urls": [_raw_url(repo, p) for p in (cand.get("paths") or [cand.get("path")])],
...
def _scan_task() -> None:
    ...
    try:
        _update(
            status="running", phase="searching", step="搜索 GitHub 仓库…", error="", finished_at=""
        )
        repos = find_repos()
        ...
        for i, repo in enumerate(repos):
            ...
            for p in pixels:
                if _SCAN["cancel"]:
                    ...
                    return
                _update(
                    candidates=_SCAN["candidates"] + 1, step=f"试转 {repo['full_name']}:{p['path']}"
                )
                td = tmp_root / f"r{i}p"
                try:
        ...
        _update(status="done", phase="done", step="扫描完成", finished_at=time.strftime("%H:%M:%S"))
