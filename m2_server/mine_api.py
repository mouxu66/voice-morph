...
def _mine_worker_thread(params: dict | None = None):
    ...
    params = params or {}
    MINE_STATE.update(
        running=True, stage="running", message="正在转写与提取声纹…", kept=0, clusters=[]
    )
    try:
        ...
        result = analyze(
            clips,
            sim_threshold=params.get("sim_threshold"),
...
def mine_run(req: MineRunRequest | None = None):
    ...
    threading.Thread(
        target=_mine_worker_thread,
        daemon=True,
        kwargs={"params": req.model_dump(exclude_none=True) if req else {}},
    ).start()
    return {"ok": True}
