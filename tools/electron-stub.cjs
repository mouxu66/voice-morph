// electron 桩装配器（测试公共件，本身不是测试：文件名不带 `test-` 前缀）
//
// 为什么需要单独一份
// ------------------
...
// （即跑过 `npm install`）。于是：
//   · CI 的 backend job 不装 npm 包 → 4 个 node 测试在 CI 上根本跑不起来
//   · 结果就是这些测试**长期只在本机凭自觉跑**，烂了也没人知道
...
//     const stubId = installElectronStub({ app: { ... }, dialog: { ... } });
